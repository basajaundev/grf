...
inline void timer_start() { gettimeofday((struct timeval *)&tv, NULL); }
...
void test_new_handler() {
  ...
  if (sizeof(struct grf_header) != GRF_HEADER_SIZE) {
    printf("Your compiler didn't use the right size for the packed structure (%x!=%x) !", (uint32_t)sizeof(struct grf_header),
           GRF_HEADER_SIZE);
    exit(2);
  }
  ...
  printf(
      " - test_new_handler(): %d byte(s) wasted (should be slightly lower than before,"
      "as table was moved to an optimized place)\n",
      grf_wasted_space(handler));
